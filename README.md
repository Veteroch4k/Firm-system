...
* **🔐 HashiCorp Vault (для секретов):**
  Используется для безопасного хранения чувствительных данных (пароли от БД, API-ключи, токены). Spring Cloud Config может работать в связке с Vault, подтягивая обычные настройки из Git, а секреты — из защищенного хранилища. 
